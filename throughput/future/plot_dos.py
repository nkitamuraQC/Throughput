...
def plot_dos_(
    filename_list, outname, choice1=2, choice2=5, range_=[-10, 0], fermi_list=None
):
    dos_list = []
    e_list = []
    count = 0
    for fn in filename_list:
        ...
        e, dos = read_dos(
            fn, choice1=choice1, choice2=choice2, range_=range_, fermi=fermi
        )
        dos_list.append(dos)
        e_list.append(e)
        count += 1
    ...
    pltcls.plot_all(
        [e_list[0]], dos_list, labelx="energy", labely="DOS", filename=outname
    )
    return
...
def plot_dos(
    ...
    pltcls.legend = orb_list
    pltcls.plot_all(
        [e_list[0]], dos_list, labelx="Energy (eV)", labely="PDOS", filename=outname
    )
    return
...
def main():
    ...
    orb_list = [
        r"PF$_6$ F 2p$_y$ orbital",
        r"AsF$_6$ F 2p$_y$ orbital",
        r"SbF$_6$ F 2p$_y$ orbital",
        r"TaF$_6$ F 2p$_y$ orbital",
    ]
    count = 0
    plot_dos(
        filename_list, orb_list, outname, fermi_list=fermi_list, range_=[-10.0, 0.0]
    )
    return
...
def main2_fermi():
    ...
    count = 0
    plot_dos_log(
        filename_list, orb_list, outname, fermi_list=fermi_list, range_=[-0.8, 0.2]
    )
    return
